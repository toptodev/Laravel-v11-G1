				...
				<div class="tile-title-w-btn">
						...
						<p>
								<a class="btn btn-primary icon-btn"
										href="{{ route('admin.banners.create') }}"><i class="fa fa-plus-circle fa-fw"></i>สร้างรายการ </a>
										...
										<tbody>
												@foreach ($banners as $index => $banner)
														<tr>
																...
																<th scope="row"
																		width="1">{{ $banners->firstItem() + $index }}</th>
																<td class="text-nowrap"
																		width="100">{{ $banner->code }}</td>
																<td class="text-nowrap">{{ $banner->title }}</td>
																<td class="text-nowrap"
																		width="100">{{ $banner->created_at }}</td>
																<td width="1">
																		<div class="option-link text-nowrap">
																				<form action="{{ route('admin.banners.destroy', $banner->id) }}"
																						method="POST">
																						<a class="btn btn-sm btn-warning"
																								href="{{ route('admin.banners.show', [$banner->id]) }}"> ดูเพิ่มเติม</a>
																						<a class="btn btn-sm btn-info"
																								href="{{ route('admin.banners.edit', [$banner->id]) }}"> แก้ไข</a>
																						@csrf
																						{{ method_field('DELETE') }}
